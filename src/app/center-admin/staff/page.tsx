...
export default function BranchStaffPage() {
  ...
  const fetchStaffTypes = async () => {
    try {
      ...
      console.error('Failed to load staff types:', err)
    }
  }
  ...
  const handleAddWorker = async () => {
    ...
    try {
      ...
      const response = await branchApi.addWorker({
        ...
        staffTypeId: formData.staffTypeId || undefined,
        workerType: formData.staffTypeId ? undefined : formData.workerType
      })
      if (response.success) {
        toast.success('Worker added successfully')
        setShowAddModal(false)
        setFormData({ name: '', email: '', phone: '', password: '', staffTypeId: '', workerType: 'general' })
        fetchStaff()
      }
    } catch (err: any) {
  ...
  const openEditModal = (worker: StaffMember) => {
    ...
    setFormData({
      ...
      phone: worker.phone || '',
      password: '',
      staffTypeId: worker.staffType?._id || '',
  ...
  const filteredStaff = staff.filter(member => {
    ...
    const matchesType = typeFilter === 'all' || 
                       member.staffType?._id === typeFilter ||
                       member.workerType === typeFilter
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {member.staffType ? (
                          <span 
                            ...
                            style={{ 
                              backgroundColor: member.staffType.color + '20',
                              color: member.staffType.color
                            }}
                          >
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFormData({ ...formData, staffTypeId: e.target.value })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                >
