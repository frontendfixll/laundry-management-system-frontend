      ...
      <div className="relative" style={{ width: size, height: size }}>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center z-20">
          <div className={`transition-all duration-300 text-center ${hoveredIndex !== null ? 'scale-110' : ''}`}>
            {hoveredIndex !== null ? (
              <>
                <p className="text-5xl font-bold" style={{ color: data[hoveredIndex].color }}>{data[hoveredIndex].value}</p>
                <p className="text-sm text-gray-600 font-semibold mt-1">{data[hoveredIndex].label}</p>
...
export default function TenantDashboard() {
  ...
  // Fetch orders for this tenant only
  useEffect(() => {
    const fetchTenantOrders = async () => {
      ...
      try {
        ...
        if (data.success) {
          ...
          const ordersArray = data.data.data || data.data.orders || []
        ...
        <header className="bg-white border-b border-gray-200 sticky top-0 z-30">
          <div className="flex items-center justify-between px-4 lg:px-8 py-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl lg:text-2xl font-bold text-gray-800">{greeting}, {user?.name?.split(' ')[0]}! 👋</h1>
        ...
        <main className="flex-1 p-4 lg:p-8 space-y-6 overflow-y-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white rounded-2xl p-6 shadow-xl border border-gray-100">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h3 className="font-bold text-gray-800">Order Distribution</h3>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              { href: `/${tenant}`, icon: Star, label: 'Store', desc: 'Back to store', gradient: 'from-amber-500 to-orange-500' },
              ...
              <Link key={i} href={item.href}>
                <div className="group bg-white rounded-2xl p-5 border border-gray-100 shadow-lg hover:shadow-xl transition-all hover:-translate-y-1">
                  <div className={`w-12 h-12 bg-gradient-to-br ${item.gradient} rounded-xl flex items-center justify-center mb-4 shadow-lg group-hover:scale-110 transition-transform`}>
          ...
          {/* Promo Banner for First-time Users */}
          {orders.length === 0 && (
