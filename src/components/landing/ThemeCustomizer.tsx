...
export default function ThemeCustomizer({
  ...
  const selectedLanguage = languages.find(l => l.id === currentLanguage) || languages[0]

  // Load saved positions and expanded state from localStorage
  useEffect(() => {
    ...
    <>
      ...
      >
        <div className={`bg-white rounded-2xl shadow-2xl border border-gray-200 ${isColorDragging ? 'shadow-2xl scale-105' : ''} transition-all`}>
          ...
          <div 
            className="flex items-center gap-2 p-3 cursor-pointer hover:bg-gray-50 rounded-t-2xl"
          >
            {/* Drag Handle */}
            <div 
            ...
            <div className="p-3 pt-0 border-t border-gray-100">
              ...
              <div className="grid grid-cols-2 gap-2 mt-2">
                {colors.map((color) => (
                  <button
                    key={color.id}
                    onClick={() => !isColorDragging && onColorChange(color.id as ThemeColor)}
              ...
              <div className="mt-3 pt-3 border-t border-gray-100">
                ...
                <div className="space-y-1">
                  {languages.map((lang) => (
                    <button
                      key={lang.id}
                      onClick={() => !isColorDragging && onLanguageChange(lang.id as Language)}
